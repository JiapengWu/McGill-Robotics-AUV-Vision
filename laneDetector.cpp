...
float rhoDiffLowerThresh = 40;
float rhoDiffUpperThresh = 100;
...
void LaneDetector::imageCallback(const sensor_msgs::Image::ConstPtr& msg)
{
  ...
  for(int i = 0; i < numCluster-1; i++){
      ...
      for(int j = i + 1; j < numCluster; j++){
          ...
          printf("rhoDiff %d-%d = %f\n", i,j,rhoDiff);
          if( thetaDiff < thetaDiffThresh && rhoDiff > rhoDiffLowerThresh && rhoDiff < rhoDiffUpperThresh){
